...
define([
	...
	var Dialog = Class.create({
				...
				template: '\
					<div class="ui-dialog  <%=customClassName%>">\
						<div class="title <%if(isDrag){%>ui-drag<%}%>">\
							<%if(isClose){%>\
							<a class="dialog-close" href="javascript:void(0);"><i class="wk_iconfont icon-duibiguanbifangyuan"></i></a>\
							<%}%>\
							<h3><%=title%></h3>\
						</div>\
						<div class="wrap">\
						...
						<%if(confirm || cancel){%>\
						<div class="buttons <%if(confirm && !cancel){%>only<%}%> clearfix">\
							<%if(cancel){%>\
								<div class="dialog-cancel clearfix"><%=cancel%></div>\
							<%}%>\
							<%if(confirm){%>\
								<div class="dialog-ok clearfix"><%=confirm%></div>\
							<%}%>\
						</div>\
						<%}%>\
					</div>\
		...
		resetHeight:function(str){
			this.container.css(str,'auto');
			this.resizeHandler();
		},
