	...
	*/
	$.fn.slotMachine = function(settings){
		...
		*/
        ...
        $slot.shuffleToActive = function(count,index,oncomplete){
			...
			_shuffleToActive(count,index);

		}

		function _shuffleToActive(count,index){
			...
			//Stop animation after {count} repeats
				...
				if( count>=1 ){
					...
					setTimeout(function(){
						...
						//Repeat animation
						_shuffleToActive( count-1,index );
				  ...
				  var nextIndex = index<$titles.length ? index : 0;
				  var rnd = {
					 index	: nextIndex,
					 el		: $titles.get(nextIndex)
				  };
